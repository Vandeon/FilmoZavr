...
public class QuickReplyServiceImpl implements QuickReplyService {
	...
	@Value("${genre_was_added}")
	String genreWasAdded;

	@Override
	public void doIt(User user, String incomePayload) {
		if (user.getUserStatus().equals(UserStatus.RAITING_FILM)) {
			rateService.addOrUpdateRate(Integer.parseInt(incomePayload), user,
					filmService.findOne(Integer.parseInt(user.getCommentingFilmId())));
			...
			} else if (incomePayload.equals(offerFilms)) {
				outputMessageService.offerFilms(user);
			} else if (genreService.ifGenre(incomePayload)) {
				addGenreFromPayloadAndOffer(user, incomePayload);
	...
	private void addGenreFromPayloadAndOffer(User user, String incomePayload) {
		userService.addGenreToUser(user, incomePayload);
		outputMessageService.sendTextMessage(user, incomePayload + " " + genreWasAdded);
		outputMessageService.offerGenres(user);
	}
